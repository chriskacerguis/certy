...
exports.renderPage = async (req, res, next) => {
  try {
    ...
    const lifecycleEnabled =
      String(process.env.ENABLE_CA_LIFECYCLE || "").toLowerCase() === "true";
    ...
    const hasKeystoreSecretOld =
      String(process.env.KEYSTORE_SECRET_OLD || "").trim().length > 0;
    ...
    res.render("ca", {
      ...
      dbPath: DB_PATH,
      caDir: CA_DIR,
      migrateJson: String(process.env.MIGRATE_JSON || "false"),
      hasKeystoreSecretOld,
    });
  } catch (e) {
    next(e);
  }
...
exports.initCA = async (req, res, next) => {
  try {
    if (
      ...
      const err = new Error(
        "CA lifecycle operations are disabled. Set ENABLE_CA_LIFECYCLE=true to allow.",
      );
      err.status = 403;
      err.expose = true;
      throw err;
    ...
    if (!errors.isEmpty()) {
      return res
        .status(400)
        .render("layout", {
          body: `<div class="alert alert-danger">${errors.array()[0].msg}</div>`,
        });
    }
    ...
    res.render("layout", {
      body: '<div class="alert alert-success">CA initialized.</div>',
    });
  } catch (e) {
    next(e);
  }
...
exports.destroyCA = async (req, res, next) => {
  try {
    ...
    res.render("layout", {
      body: '<div class="alert alert-warning">CA data wiped (database tables cleared and local CA directory removed).</div>',
    });
  } catch (e) {
    next(e);
  }
...
exports.downloadRoot = async (req, res, next) => {
  try {
    ...
    if (!ok) {
      const err = new Error(
        "The CA is not initialized yet. Initialize the CA before downloading root certificates.",
      );
      err.status = 409;
      err.expose = true;
      throw err;
...
exports.downloadIntermediate = async (req, res, next) => {
  try {
    ...
    if (!ok) {
      const err = new Error(
        "The CA is not initialized yet. Initialize the CA before downloading intermediate certificates.",
      );
      err.status = 409;
      err.expose = true;
      throw err;
...
exports.downloadCRL = async (req, res, next) => {
  try {
    ...
    if (!ok) {
      const err = new Error(
        "The CA is not initialized yet. Initialize the CA before downloading a CRL.",
      );
      err.status = 409;
      err.expose = true;
      throw err;
...
exports.publishCRLToS3 = async (req, res, next) => {
  try {
    ...
    if (!ok) {
      const err = new Error(
        "The CA is not initialized yet. Initialize the CA before publishing a CRL.",
      );
      err.status = 409;
      err.expose = true;
      throw err;
    ...
    if (!crlPublisher.isEnabled()) {
      const err = new Error(
        "S3 CRL publishing is not enabled. Set S3_CRL_ENABLE=true and required S3 vars.",
      );
      err.status = 400;
      err.expose = true;
      throw err;
