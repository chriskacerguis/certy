...
exports.issueCertificate = async (req, res, next) => {
  try {
    ...
    addFlash(
      req,
      "success",
      `Certificate for ${commonName} generated. Your download should begin shortly.`,
    );

    // Return a ZIP with separate files
    const archiver = require("archiver");
    res.setHeader("Content-Type", "application/zip");
    const safeCN = String(commonName || "certificate").replace(
      /[^a-zA-Z0-9_.-]+/g,
      "_",
    );
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="${safeCN}.zip"`,
    );
...
exports.issueSmime = async (req, res, next) => {
  try {
    ...
    const intermediatePem = await step.fetchIntermediatesPEM();
    const p12Buffer = await createP12({
      certPem,
      privateKeyPem,
      caPem: intermediatePem,
      password,
    });
    ...
    if (String(sendEmail || "").toLowerCase() === "on") {
      ...
      if (!smtpHost) {
        ...
        audit.event("EMAIL_SMIME_BLOCKED", {
          to: email,
        ...
        try {
          ...
          audit.event("EMAIL_SMIME_SENT", {
            to: email,
            attachment: `${email}.p12`,
          });
        } catch (mailErr) {
...
exports.renderRenewPage = async (req, res, next) => {
  try {
    ...
    if (serial) {
      ...
      const row = db
        .prepare("SELECT cert_pem FROM certs WHERE serial_hex=?")
        .get(serial);
      if (row && row.cert_pem) {
        ...
        const latest = db
          .prepare(
            "SELECT cert_pem FROM certs WHERE renewed_from=? ORDER BY not_after DESC LIMIT 1",
          )
          .get(serial);
        if (latest && latest.cert_pem) certPemPrefill = latest.cert_pem;
      }
    }
    res.render("certs/renew", {
      csrfToken: res.locals.csrfToken,
      certPemPrefill,
      keyPemPrefill,
      serial,
...
exports.renewCertificate = async (req, res, next) => {
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      'attachment; filename="renewed.crt.pem"',
    );
    res.send(renewedPem);
  } catch (e) {
    next(e);
...
exports.renderRevokePage = async (req, res, next) => {
  try {
    res.render("certs/revoke", { csrfToken: res.locals.csrfToken });
  } catch (e) {
    next(e);
  }
};
...
exports.revokeCertificate = async (req, res, next) => {
  try {
    ...
    const serial =
      req.body.serial || (certPem ? extractSerialDecimal(certPem) : "");
    ...
    if (
      req.accepts("json") &&
      (req.get("x-requested-with") === "fetch" || req.xhr)
    ) {
      return res.json({ ok: true, serial });
    }
    res.render("layout", {
      body: `<div class="alert alert-warning">Certificate revoked (serial ${serial}).</div>`,
    });
  } catch (e) {
    next(e);
  }
...
exports.getCertificateBySerial = async (req, res, next) => {
  try {
    ...
    const row = db
      ...
      .get(serial);
    if (!row || !row.cert_pem)
      return res.status(404).json({ error: "Certificate PEM not found" });
...
exports.downloadCertificateBySerial = async (req, res, next) => {
  try {
    const serial = String(req.params.serial || "").trim();
    if (!serial) {
      const err = new Error("Missing serial");
      err.status = 400;
      err.expose = true;
      throw err;
    }
