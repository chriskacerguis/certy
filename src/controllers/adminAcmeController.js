...
const TABS = /** @type {const} */ (['orders', 'accounts', 'authzs', 'challenges']);
...
exports.list = (req, res, next) => {
  try {
    ...
    const counts = {
      ...
      accounts: db.prepare('SELECT COUNT(*) AS c FROM acme_accounts').get().c || 0,
      authzs: db.prepare('SELECT COUNT(*) AS c FROM acme_authzs').get().c || 0,
      challenges: db.prepare('SELECT COUNT(*) AS c FROM acme_challenges').get().c || 0,
    };
  ...
  return res.render('admin/acme', {
    ...
    if (tab === 'accounts') {
      ...
      ` : '';
      total = db.prepare(`SELECT COUNT(*) AS c FROM acme_accounts a ${where}`).get({ like }).c || 0;
      const { totalPages, curPage, offset, pages } = pager(total, pageSize, page);
      rows = db.prepare(
