(function () {
  ...
  function setFlash(type, html) {
    const cls =
      type === "error" ? "danger" : type === "warn" ? "warning" : "success";
    flash.innerHTML =
      '<div class="alert alert-' + cls + '" role="alert">' + html + "</div>";
  }
  ...
  btn.addEventListener("click", function () {
    ...
    setFlash(
      ...
      "Generating certificate for " +
        cn.replace(/</g, "&lt;") +
        "… your download will start when ready.",
    );
  });
  ...
  form.addEventListener("submit", async function (ev) {
    try {
      ...
      if (!res.ok) {
        ...
        setFlash(
          "error",
          "Issuance failed. " + (text ? "Details: " + text : ""),
        );
        return;
      }
      ...
      setFlash(
        ...
        "Certificate for " +
          cn.replace(/</g, "&lt;") +
          " generated. Your download should begin shortly.",
      );
      ...
      try {
        ...
        const r = await fetch("/certs/list.json?" + paramsNow.toString(), {
          credentials: "same-origin",
        });
        if (r.ok) {
          const j = await r.json();
          if (listWrap && j && Array.isArray(j.rows)) {
            listWrap.innerHTML = renderIssuedTable(
              j,
  ...
  document.addEventListener("submit", async function (ev) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setFlash(
          "error",
          "Revocation failed. " + (text ? "Details: " + text : ""),
        );
        return;
      }
  ...
  function renderIssuedTable(data, csrfToken) {
    ...
    html +=
      "    <thead><tr><th>Serial</th><th>CN</th><th>Not Before</th><th>Not After</th><th>SANs</th><th>Revoked</th></tr></thead>\n";
    html += "    <tbody>\n";
    if (!rows.length) {
      html +=
    ...
    for (const r of rows) {
      ...
      html +=
        "        <td><code>" +
        escapeHtml(r.serial_hex || "") +
        "</code></td>\n";
      html += "        <td>" + escapeHtml(r.subject_cn || "") + "</td>\n";
      html +=
        "        <td>" +
        escapeHtml((r.not_before || "").replace("T", " ").replace("Z", "")) +
        "</td>\n";
      html +=
        "        <td>" +
        escapeHtml((r.not_after || "").replace("T", " ").replace("Z", "")) +
        "</td>\n";
      html +=
        '        <td class="text-truncate" style="max-width: 280px" title="' +
        escapeHtml(sanStr) +
        '">' +
        escapeHtml(sanStr) +
        "</td>\n";
      ...
      if (r.revoked_at) {
        ...
        html +=
          '          <div class="small text-muted">' +
          escapeHtml((r.revoked_at || "").replace("T", " ").replace("Z", "")) +
          "</div>\n";
      } else {
        ...
        html += "            </form>\n";
        html +=
          '            <a class="btn btn-sm btn-outline-secondary" href="/certs/renew?serial=' +
          encodeURIComponent(r.serial_hex || "") +
          '" data-action="go-renew">Renew</a>\n';
        html +=
          '            <a class="btn btn-sm btn-outline-primary" href="/certs/by-serial/' +
          encodeURIComponent(r.serial_hex || "") +
          '.pem">Download Cert</a>\n';
        html += "          </div>\n";
      }
    ...
    if (totalPages > 1) {
      ...
      html +=
        ...
        '"><a class="page-link" href="' +
        (page === totalPages ? "#" : base + "&page=" + (page + 1)) +
        '">Next</a></li>\n';
      html += "  </ul></nav>\n";
    }
    html += "</div>\n";
