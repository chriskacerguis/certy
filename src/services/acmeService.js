...
const ACME_ENABLE =
  String(process.env.ACME_ENABLE || "false").toLowerCase() === "true";
...
async function verifyJWS(jws, getAccountByKid) {
  ...
  const payload = jws.payload
    ? JSON.parse(b64uToBuf(jws.payload).toString("utf8"))
    : {};
...
async function newOrder(req, res) {
  ...
  identifiers.forEach((i) => {
    if (i.type !== "dns") throw acmeErr("unsupportedIdentifier", 400);
  });
  ...
  const order = tx(() => {
    ...
    db.prepare(
      `UPDATE acme_orders SET finalize_url=?, cert_url=? WHERE id=?`,
    ).run(finUrl, cUrl, id);
    ...
    for (const ident of identifiers) {
      ...
      db.prepare(`UPDATE acme_authzs SET url=? WHERE id=?`).run(
        authzFinalUrl,
        authzId,
      );
...
function getAuthz(req, res) {
  ...
  const ch = db
    .prepare(`SELECT * FROM acme_challenges WHERE authz_id=?`)
    .get(id);
...
async function postChallenge(req, res) {
  ...
  tx(() => {
    ...
    db.prepare(`UPDATE acme_authzs SET status=? WHERE id=?`).run(
      ok ? "valid" : "pending",
      a.id,
    );
    // If all authzs valid, bump order to ready
    const row = db
      .prepare(`SELECT order_id FROM acme_authzs WHERE id=?`)
      .get(a.id);
    const remaining = db
      .prepare(
        `SELECT COUNT(*) AS c FROM acme_authzs WHERE order_id=? AND status!='valid'`,
      )
      .get(row.order_id);
    if (remaining.c === 0) {
      db.prepare(`UPDATE acme_orders SET status='ready' WHERE id=?`).run(
        row.order_id,
      );
    }
  });
...
async function finalize(req, res) {
  ...
  if (order.status !== "ready" && order.status !== "pending")
    throw acmeErr("orderNotReady", 403, "order not ready");
  ...
  tx(() => {
    db.prepare(
      `UPDATE acme_orders SET status='valid', csr_der_b64u=?, cert_pem=? WHERE id=?`,
...
async function getCert(req, res) {
  ...
  if (!row || !row.cert_pem)
    throw acmeErr("malformed", 404, "certificate not ready");
