...
exports.event = (type, details = {}) => {
  ...
  if (ctx && ctx.user)
    meta.user = { id: uid, name: ctx.user.name, email: ctx.user.email };
  if (ctx && ctx.ip) meta.ip = ctx.ip;
  const safe = redact({ ...meta, ...details });
  // DB
  try {
    db.prepare(
      "INSERT INTO audit_logs(ts, type, details_json, user_id, user_name, user_email, ip) VALUES(?, ?, ?, ?, ?, ?, ?)",
    ).run(
      ts,
      String(type || ""),
      JSON.stringify(safe),
...
function startAuditRetentionJob() {
  ...
  try {
    ...
    if (removed > 0)
      logger.info({ removed }, "audit retention purge on startup");
  } catch (e) {
