...
exports.createP12 = async ({ certPem, privateKeyPem, caPem, password }) => {
  ...
  const caCerts = [];
  if (caPem) {
    const parts = String(caPem).match(/-----BEGIN CERTIFICATE-----[\s\S]*?-----END CERTIFICATE-----/g) || [];
  ...
  const p12Asn1 = pkcs12.toPkcs12Asn1(key, cert, password, { algorithm: '3des', friendlyName: 'S/MIME', certChain: caCerts });
