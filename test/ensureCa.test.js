describe('middleware/ensureCa', () => {
  ...
  test('calls next() when CA is initialized', async () => {
    jest.doMock('../src/services/stepCaService', () => ({ isInitialized: jest.fn(async () => true) }), { virtual: false });
  ...
  test('passes 409 error when CA is not initialized', async () => {
    jest.doMock('../src/services/stepCaService', () => ({ isInitialized: jest.fn(async () => false) }), { virtual: false });
    const ensureCa = require('../src/middleware/ensureCa');
  ...
  test('propagates unexpected errors', async () => {
    jest.doMock('../src/services/stepCaService', () => ({ isInitialized: jest.fn(async () => { throw new Error('boom'); }) }), { virtual: false });
    const ensureCa = require('../src/middleware/ensureCa');
