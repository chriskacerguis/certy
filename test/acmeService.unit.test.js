...
function jws(protectedHeader, payload) {
  const p = Buffer.from(JSON.stringify(protectedHeader), "utf8").toString(
    "base64url",
  );
  const pl =
    payload !== undefined
...
describe("acmeService unit", () => {
  ...
  test("newAccount creates account and returns 201", async () => {
    ...
    const req = {
      ...
      secure: false,
      acmeJws: jws(
        { nonce, alg: "RS256", jwk: { kty: "RSA", n: "x", e: "AQAB" } },
        { contact: [] },
      ),
    };
    const out = {};
  ...
  test("bad nonce yields ACME error", async () => {
    ...
    const req = {
      ...
      secure: false,
      acmeJws: jws(
        { nonce: "nope", alg: "RS256", jwk: { kty: "RSA", n: "x", e: "AQAB" } },
        {},
      ),
    };
    const res = {
    ...
    await expect(svc.newAccount(req, res)).rejects.toHaveProperty(
      "acme.type",
      "badNonce",
    );
  });
