...
describe("acmeController", () => {
  ...
  test("returns 404 for all endpoints when ACME is disabled", async () => {
    ...
    await agent
      .post("/acme/challenge/1")
      .set("Content-Type", "application/jose+json")
      .send(Buffer.from("{}"))
      .expect(404);
    await agent
      .post("/acme/finalize/1")
      .set("Content-Type", "application/jose+json")
      .send(Buffer.from("{}"))
      .expect(404);
  ...
  test("delegates to acmeService and sets headers when ACME is enabled", async () => {
    ...
    jest.doMock(
      "../src/services/acmeService",
      () => {
        const m = {
          ACME_ENABLE: true,
          issueNonce: jest.fn(() => "nonce123"),
          directory: jest.fn((_req) => ({ newNonce: "http://x/new-nonce" })),
          headNewNonce: jest.fn((res) => {
            res.set("Replay-Nonce", "head-nonce");
            res.status(200).end();
          }),
          newAccount: jest.fn(async (_req, res) => {
            res.status(201).json({ ok: "account" });
          }),
          newOrder: jest.fn(async (_req, res) => {
            res.status(201).json({ ok: "order" });
          }),
          getAuthz: jest.fn((req, res) => {
            res.set("Replay-Nonce", "nonce2");
            res.json({ ok: "authz", id: req.params.id });
          }),
          postChallenge: jest.fn(async (_req, res) => {
            res.set("Replay-Nonce", "nonce3");
            res.json({ ok: "challenge" });
          }),
          finalize: jest.fn(async (_req, res) => {
            res.set("Replay-Nonce", "nonce4");
            res.status(200).json({ ok: "finalize" });
          }),
          getCert: jest.fn(async (_req, res) => {
            res.type("application/pem-certificate-chain").send("PEM");
          }),
        };
        return m;
      },
