...
function freshEnv(extra = {}) {
  for (const k of ["LOCAL_CA_DIR", "LOCAL_CA_DB", "ENABLE_CA_LIFECYCLE"])
...
describe("App E2E routes", () => {
  ...
  test("Issue TLS -> returns a zip with separate files", async () => {
    ...
    const zipRes = await agent
      ...
      .parse(binaryParser)
      .send(
        `_csrf=${csrf}&commonName=test.local&sans=test.local&days=30&keyType=RSA`,
      )
      .expect(200)
    ...
    expect(names.sort()).toEqual(
      ["certificate.pem", "chain.pem", "private.key", "request.csr"].sort(),
    );
