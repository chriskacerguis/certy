  ...
  test("newAccount -> newOrder -> authz -> challenge -> finalize -> getCert", async () => {
    ...
    jest.doMock(
      ...
      () => ({
        ...
        FlattenedVerify: jest
          .fn()
          .mockImplementation(() => ({ verify: async () => ({}) })),
      }),
      { virtual: false },
    );
    ...
    global.fetch = jest.fn(async () => ({
      ok: true,
      text: async () => global.__ACME_KEY_AUTH__ || "",
    }));
    ...
    const jws2 = makeJws(
      { nonce: nonce2, alg: "RS256", kid },
      { identifiers: [{ type: "dns", value: "example.com" }] },
    );
    const order = await agent
      .post("/acme/new-order")
      .set("Content-Type", "application/jose+json")
      .send(Buffer.from(JSON.stringify(jws2)))
      .expect(201);
    ...
    const ch = await agent
      .post(`/acme/challenge/${authzId}`)
      .set("Content-Type", "application/jose+json")
      .send(Buffer.from(JSON.stringify(jws3)))
      .expect(200);
    ...
    const jws4 = makeJws(
      { nonce: nonce4, alg: "RS256", kid },
      { csr: csrDerB64u },
    );
    const fin = await agent
      .post(`/acme/finalize/${orderId}`)
      .set("Content-Type", "application/jose+json")
      .send(Buffer.from(JSON.stringify(jws4)))
      .expect(200);
